...
function buildYAxis(points: HeartRatePoint[]) {
    const values = points.flatMap(point => [point.min, point.max]).filter(value => value > 0);
    const peak = values.length ? Math.max(...values) : 100;
    const floor = values.length ? Math.min(...values) : 60;
    const min = Math.max(0, Math.floor((floor - 10) / Y_AXIS_INTERVAL) * Y_AXIS_INTERVAL);
    const max = Math.max(
        min + Y_AXIS_INTERVAL * 4,
        Math.ceil((peak + 10) / Y_AXIS_INTERVAL) * Y_AXIS_INTERVAL,
    );
...
function isValidPoint(point: HeartRatePoint) {
...
type Props = {
    ...
    data?: HeartRatePoint[];
    onPointChange?: (point: HeartRatePoint | undefined) => void;
};
...
function parsePointX(point: HeartRatePoint) {
...
function getCategoryGridExtensionPositions(range: HeartRateChartRange, labels: string[]) {
    if (range === 'month') {
...
function buildHeartRateLineSeries(
...
const TODAY_DEMO_DATA: HeartRatePoint[] = [
    { hour: '08:00', min: 62, max: 85, x: 8 },
    { hour: '12:00', min: 68, max: 95, x: 12 },
    { hour: '18:00', min: 65, max: 88, x: 18 },
];

const WEEK_DEMO_DATA: HeartRatePoint[] = getWeekLabels().map((label, index) => {
    const min = [58, 62, 60, 65, 59, 61, 63][index];
    const max = [82, 95, 88, 92, 80, 86, 90][index];
    return { hour: label, min, max };
});

const MONTH_DEMO_DATA: HeartRatePoint[] = getMonthLabels().map((label, index) => {
    if (index % 5 === 0) {
        return { hour: label, min: 0, max: 0 };
    }
    const min = 58 + (index % 5);
    const max = min + 20 + (index % 4) * 3;
    return { hour: label, min, max };
});

function getDefaultData(range: HeartRateChartRange) {
    switch (range) {
        case 'week':
...
export default function HeartRateDetailChart({ range, data, onPointChange }: Props) {
