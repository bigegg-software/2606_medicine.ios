...
export default function MessagePage() {
    ...
    const mapRows = useCallback(
        (rows: PatientMessageItem[]) =>
            rows.map(item => buildMessageListDisplayItem(item, identityPerspective)),
        [identityPerspective],
    );
    ...
    const loadPage = useCallback(
        async (page: number, mode: 'initial' | 'refresh' | 'loadMore') => {
            ...
            try {
                ...
                const rows = getResourceRows<PatientMessageItem>(res as { code?: number; rows?: PatientMessageItem[] });
    ...
    const handleReadAll = useCallback(async () => {
        if (markingAll || !messageScope.userIds) return;
        try {
            setMarkingAll(true);
