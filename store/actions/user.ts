...
import { getUserInfo, type UserExtr } from '@/api/user';
import { saveUserId } from '@/services/storage';
import { isResourceApiOk } from '@/src/utils/apiHelpers';
import { CLEAR_USER, SET_USER, SET_USER_EXTR, SET_USER_LOADING } from '../type/user';
import type { RootState } from '../store';

type AppDispatch = (action: { type: string; payload?: unknown }) => unknown;

async function applyUserExtr(dispatch: AppDispatch, res: unknown) {
  if (!isResourceApiOk(res as { code?: number })) return;
  const userExtr = (res as { data?: { userExtr?: UserExtr } }).data?.userExtr;
  if (!userExtr) return;
  if (userExtr.userId != null) await saveUserId(userExtr.userId);
...
export const fetchUserInfo = () => async (dispatch: AppDispatch) => {
  try {
    ...
    await applyUserExtr(dispatch, res);
  } catch {
    /* ignore */
  }
};
...
export const fetchUserSession = () => async (dispatch: AppDispatch) => {
  ...
  try {
    ...
    await Promise.all([
      systemRes ? applyUserExtr(dispatch, systemRes) : Promise.resolve(),
