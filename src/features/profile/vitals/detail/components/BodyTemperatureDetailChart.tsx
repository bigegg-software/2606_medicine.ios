...
export type BodyTemperatureRangePoint = {
    hour: string;
    min: number;
    max: number;
    x?: number;
...
function isValidPoint(point: BodyTemperatureRangePoint) {
...
function buildRangeBarItem(point: BodyTemperatureRangePoint, rangeValue: number | number[]) {
    return {
        value: rangeValue,
        name: point.hour,
        itemStyle: {
            color: getBarColor(point.min, point.max),
...
function buildIntervalCapSeries(
    ...
    getX: (point: BodyTemperatureRangePoint, index: number) => number,
) {
    const capData: Array<{
        value: [number, number];
    ...
    points.forEach((point, index) => {
        ...
        const color = getBarColor(point.min, point.max);
        capData.push(
            {
                value: [x, point.min],
...
function parsePointX(point: BodyTemperatureRangePoint) {
...
function getCategoryGridExtensionPositions(range: BodyTemperatureChartRange, labels: string[]) {
    if (range === 'month') {
...
const TODAY_DEMO_DATA: BodyTemperatureRangePoint[] = [
    { hour: '08:00', min: 36.2, max: 36.8, x: 8 },
    { hour: '12:00', min: 36, max: 38, x: 12 },
    { hour: '20:00', min: 35.5, max: 36.1, x: 20 },
];

const WEEK_DEMO_DATA: BodyTemperatureRangePoint[] = getWeekLabels().map((label, index) => {
    const ranges = [
        [36.2, 36.8],
        [36.5, 37.0],
        [35.8, 36.4],
...
const MONTH_DEMO_DATA: BodyTemperatureRangePoint[] = getMonthLabels().map((label, index) => {
    if (index % 5 === 0) {
        return { hour: label, min: 0, max: 0 };
    }
    const min = 35.8 + (index % 6) * 0.1;
    const max = Math.min(37.5, min + 0.4 + (index % 3) * 0.1);
    return { hour: label, min, max };
});

function getDefaultData(range: BodyTemperatureChartRange) {
    switch (range) {
        case 'week':
